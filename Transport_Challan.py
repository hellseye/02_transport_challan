...
file_path = os.path.join(os.getcwd(), "Data", "company_data.json")
...
def generate_challan(company_data, transport_data,contact_no, company_name, transport_name, items_data, discount, gst, date, challan_number,No_of_Other_Party_Goods,Amount_of_Other_Party_Goods):
    ...
    New_No_other_party_goods = 0 + No_of_Other_Party_Goods
    ws["A28"].value = New_No_other_party_goods
    ws['A28'].font = Font(bold=True, size=16)
    New_Amount_other_party_goods = 0 + Amount_of_Other_Party_Goods
    ws["F28"].value = New_Amount_other_party_goods
    ws["F28"].value = Amount_of_Other_Party_Goods
    
    ws["F28"].alignment = Alignment(wrapText=True)
