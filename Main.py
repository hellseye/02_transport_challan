...
def on_key_release(event):
    # Access the parent CTkComboBox from the underlying Entry widget.
    combo = event.widget.master  
    current_text = combo.get()
    filtered_values = [value for value in combo.all_values if current_text.lower() in value.lower()]
    combo.configure(values=filtered_values)
    
    # Automatically open the dropdown by simulating a click on the dropdown button.
    try:
        # Use the private attribute _dropdown_button if available.
        if hasattr(combo, "_dropdown_button"):
            combo._dropdown_button.event_generate("<Button-1>")
    except Exception as e:
        print("Auto open dropdown failed:", e)

def evaluate_sum_expr(expr):
    """
    Evaluates a string expression that consists of integers separated by '+'.
    For example, "34+34+45+22" returns 135.
    If the expression is empty or invalid, returns 0.
    """
    try:
        # Split the expression on '+' and sum up the integer values
        numbers = [int(part.strip()) for part in expr.split('+') if part.strip().isdigit()]
        return sum(numbers)
    except Exception as e:
...
def create_dropdown(parent, row, column, text, names):
    ...
    label = ctk.CTkLabel(parent, text=text, text_color=white, font=("Arial", 16))
    label.grid(row=row, column=column, padx=10, pady=5, sticky="w")
    dropdown = ctk.CTkComboBox(parent, values=sorted_names, width=400, font=("Arial", 16))
    dropdown.all_values = sorted_names
    dropdown.bind("<KeyRelease>", on_key_release)
    dropdown.grid(row=row, column=column + 1, padx=10, pady=5)
    return dropdown

def create_label_and_entry(parent, text, row, column, width=600, default_value=""):
    ...
    entry.insert("1.0", default_value)  # Insert default value if provided
...
def get_monthly_challan_counts(directory="generated_challans"):
    ...
    for file in os.listdir(directory):
        if file.startswith("transport_challan") and file.endswith(".xlsx"):
            ...
            # "transport_challan_ABC_Corp_XYZ_Logistics_12_12_2025_1.xlsx"
            # the parts will be:
            # parts[0] = "transport"
            # parts[1] = "challan"
...
class HomePage(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.date_text = create_label_and_entry(self, "Date (DD.MM.YY):", 5, 0, default_value=current_date)
        self.company_text.set("")
        self.transport_text.set("")
    ...
    def submit_data(self):
        ...
        no_of_other_party_goods = evaluate_sum_expr( self.no_of_other_goods.get("1.0", "end-1c").strip())
        amount_of_other_party_goods =evaluate_sum_expr(self.amount_of_other_goods.get("1.0", "end-1c").strip())
        ...
        generate_challan(
            ...
            No_of_Other_Party_Goods=int(no_of_other_party_goods),
            Amount_of_Other_Party_Goods=int(amount_of_other_party_goods)
        )
...
class AddSupplierPage(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.Company_name = create_label_and_entry(self, "Supplier Name:", 1,0)
        self.address1_text = create_label_and_entry(self, "Address(1) of supplier:", 2, 0)
        self.address2_text = create_label_and_entry(self, "Address(2) of supplier:", 3, 0)
        self.gst_text = create_label_and_entry(self, "GST no. of supplier:", 4, 0)
    ...
    def submit_supplier_data(self):
        company_name = self.Company_name.get("1.0", "end-1c")
...
class AddCustomerPage(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.Customer_name = create_label_and_entry(self, "Customer name:", 1, 0)
        self.station_text = create_label_and_entry(self, "Address of customer:", 2, 0)
        self.gst_text = create_label_and_entry(self, "GST no. of customer:", 3, 0)
        self.way_text = create_label_and_entry(self, "Transport Way:", 4, 0)
    ...
    def submit_customer_data(self):
        customer_name = self.Customer_name.get("1.0", "end-1c")
...
class StatisticsPage(ctk.CTkFrame):
    ...
    def update_graph(self):
        ...
        ax.plot(ordered_months, counts, marker='o', linestyle='-', color='deepskyblue', linewidth=2)
        ax.set_title("Challans Generated Per Month", color="white")
        ax.set_xlabel("Month", color="white")
        ax.set_ylabel("Number of Challans", color="white")

        ax.set_facecolor("#1E1E1E")  # Inside the graph
        fig.patch.set_facecolor("#121212")  # Outside the graph
